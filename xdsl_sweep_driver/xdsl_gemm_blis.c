...
size_t    ic, jc, pc, mc, nc, kc, ir, jr, mr, nr; 
  ...
  for ( jc=0; jc<n; jc+=NC ) {
    ...
    for ( pc=0; pc<k; pc+=KC ) {
      ...
      pack_CB( orderB, transB, kc, nc, Bptr, ldB, Bc, NR_arg);
	...
	//Comment or uncomment for packing or not
        pack_RB( orderA, transA, mc, kc, Aptr, ldA, Ac, MR_arg);
	    ...
	    int be = (int)betaI;
	     ...
	     ukr(NULL, kc, &alpha,
#ifdef BASE 
			     (struct exo_win_2f32c){&Ac[ir*kc],{MR_arg,1}},                                  
			     (struct exo_win_2f32c){&Bc[jr*kc],{NR_arg,1}},   
#else			     
			     &Ac[ir*kc],MR_arg,                                  
			     &Bc[jr*kc],NR_arg,   
#endif
