...
from xdsl.dialects import arith, emitc, func, scf
...
class MacroBuilder:
    ...
    def get_ptr(self, memref_val: SSAValue, offset: SSAValue) -> SSAValue:
        ...
        op = emitc.EmitC_CallOpaqueOp(
            "GET_PTR", [memref_val, offset], result_types=[ptr_t]
        )
        self.block.add_op(op)
        return op.results[0]
    ...
    def calc_offset(
        self, row: SSAValue, col: SSAValue, ld: SSAValue, layout: str
    ) -> SSAValue:
        if layout == "C":
...
def build_macro_module(
    ...
    ir_builder.call_opaque(
        "gemm_microkernel_v", [mr, nr, kc, alpha, Ac_ptr, Bc_ptr, betaI, Cptr, ldC]
    )
