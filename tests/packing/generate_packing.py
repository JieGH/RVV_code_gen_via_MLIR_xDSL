...
def verify_intrinsics(output: str, fn_name: str, expected: list, forbidden: list) -> bool:
    ok = True
    print(f"  [{fn_name}]")
    if not output:
        print(f"    ❌  FUNCTION NOT FOUND IN IR"); return False
    for intr in expected:
        if intr in output:
            print(f"    ✅  {intr}")
        else:
            print(f"    ❌  {intr}  ← MISSING"); ok = False
    for intr in forbidden:
        if intr not in output:
            print(f"    ✅  {intr} — correctly absent")
        else:
            print(f"    ❌  {intr}  ← must be absent"); ok = False
    return ok


def run(NR: int, MR: int, elem_bytes: int, suffix: str, output: str, mlir_translate: str | None):
    idx = IndexType()
    f32_memref = MemRefType(Float32Type(), [-1])
    module = pb.build_pack_module(idx, f32_memref, NR=NR, MR=MR,
                                  elem_bytes=elem_bytes, suffix=suffix)
    ctx = make_context()
    ...
    all_ok &= verify_intrinsics(fn_blocks.get(cb_if), cb_if,
                                ["__riscv_vlse32"], ["__riscv_vle32"])
    all_ok &= verify_intrinsics(fn_blocks.get(cb_else), cb_else,
                                ["__riscv_vle32"], ["__riscv_vlse32"])
    all_ok &= verify_intrinsics(fn_blocks.get(rb_if), rb_if,
                                ["__riscv_vle32"], ["__riscv_vlse32"])
    all_ok &= verify_intrinsics(fn_blocks.get(rb_else), rb_else,
                                ["__riscv_vlse32"], ["__riscv_vle32"])
...
if __name__ == "__main__":
    ...
    args = p.parse_args()
    run(args.NR, args.MR, args.elem_bytes, args.suffix, args.output, args.mlir_translate)
